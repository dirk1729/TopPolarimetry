...
C
      ...
      REAL*8 XVAR,ptmax1,ptmax2,htj,tmp,inclht, ptcut
...
c
	 ...
	 ptemp2(j)=0
      enddo	
      do j=0,3
         ptemp(j) = ptemp(j) + P(j,3)
         ptemp(j) = ptemp(j) + P(j,4)
         ptemp(j) = ptemp(j) + P(j,5)
         ptemp2(j) = ptemp2(j) + P(j,6)
         ptemp2(j) = ptemp2(j) + P(j,7)
         ptemp2(j) = ptemp2(j) + P(j,9)
      enddo
      ptcut = dsqrt(ptemp(1)**2 + ptemp(2)**2)
      if (ptcut .lt. 200d0) then
         passcuts=.false.
         return
      endif 
c	
